...
### 7-1. 예약현황
- 상단에 이용내역과 현재 예약내역을 선택할수 있는 버튼이 존재하며, 현재 이용내역의 버튼을 기본값으로 지정한다.
- 이용내역을 클릭했을 경우 이전에 내가 사용했던 내역을의 리스트가 출력되며 이전에 사용했던 내역이 없을 경우 '이용내역이 없습니다.'라는 모달창 출력한다. 

#### 7-1-1.예약이 존재하지 않는 경우
[예약이 존재하지 않는 경우]
...
- 밑칸에는 숙소예약번호, 숙소대표이미지, 숙소이름, 숙박일수, 체크인+체크아웃(대실or숙박), 동반인원을 표시한다.
